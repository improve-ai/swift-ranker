...
#   define IMPLog(fmt, ...) os_log_debug(OS_LOG_DEFAULT, ("[ImproveAI] " fmt), ##__VA_ARGS__)
#else
...
/**
 ...
 Format: "-/+[<class> <method>] L<line> <formatted message>"
 */
#define IMPErrLog(fmt, ...) os_log_error(OS_LOG_DEFAULT, ("[ImproveAI] %s L%d " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__)
